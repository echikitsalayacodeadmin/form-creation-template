...
async function replaceAgeUsingOffset(ecgUrl, newAge = "50") {
  const ageLabel = await findAgeLabelPosition(ecgUrl);
  if (!ageLabel) {
  ...
  const VALUE_X = ageLabel.x + 40; // ✅ exactly what you wanted
...
const ReplaceAgeInECG = ({
  ...
  const fetchListOfEmployees = async () => {
    ...
    if (result && result.data) {
      ...
      setList(sorted);
      console.log("Total Ecg employees:", sorted.length);
  ...
  const handleEcgModify = async (data) => {
    try {
      ...
      const found = await findAgeLabelPosition(ecgUrl);
      if (!found) {
        alert("Could not find 'Age :' in the Ecg PDF!");
        return;
      }
      ...
      const modifiedBlob = await replaceAgeUsingOffset(data.ecgUrl, "50");
...
export default ReplaceAgeInECG;
