...
async function replaceAgeUsingOffset(pftUrl, newAge = "50") {
  const ageLabel = await findAgeLabelPosition(pftUrl);
  if (!ageLabel) {
  ...
  const VALUE_X = ageLabel.x + 47; // ✅ exactly what you wanted
...
const ReplaceAgeInPft = ({
  ...
  const handleEcgModify = async (data) => {
    try {
      ...
      const found = await findAgeLabelPosition(pftUrl);
      if (!found) {
        alert("Could not find 'Age :' in the Pft PDF!");
        return;
      }
      ...
      const modifiedBlob = await replaceAgeUsingOffset(data.pftUrl, "50");
...
export default ReplaceAgeInPft;
