...
async function mergeHeaderAndXraySinglePage(headerPdfBytes, xrayPdfBytes) {
  ...
  const xrayPdf = await PDFDocument.load(xrayPdfBytes);

  const mergedPdf = await PDFDocument.create();

  // Copy the header page
  const [headerPage] = await mergedPdf.copyPages(headerPdf, [0]);
  mergedPdf.addPage(headerPage);
  const page = mergedPdf.getPage(0);
  const { width, height } = page.getSize();

  // Embed first page of the X-ray PDF
  const [xrayPage] = await xrayPdf.getPages();
  const embeddedXray = await mergedPdf.embedPage(xrayPage);

  const xrayWidth = width;
  const xrayHeight = height - 150; // adjust this value if needed

  // Draw the embedded X-ray page on the same page below the header
  page.drawPage(embeddedXray, {
    x: 0,
...
const StridesXrayFormMain = ({
  ...
  //     formData.append("file", pdfBlob, `${data?.empId}_xrayFilmUrl.pdf`);
      ...
      <div>
        ...
        {filteredList.map((item, index) => (
          <div key={index} style={{ display: "flex" }}>
            ...
            <a href={item.xrayUrl}>
              <div key={index}>{item.xrayUrl}</div>
            </a>
            <br />
          </div>
        ))}
...
export default StridesXrayFormMain;
